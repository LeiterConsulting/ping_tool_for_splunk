...
<#
.SYNOPSIS
    Splunk Ping Monitor v4.0.0 - Bounded parallel scheduler + memory stability.
...
.DESCRIPTION
    Pings endpoints from CSV, outputs to file (UF) or Splunk HEC, with optional metrics.
        ...
        ========================
        MEMORY STABILITY: Bounded parallel scheduler
            - Limits in-flight runspaces to parallel_threads (RunspacePool.MaxRunspaces)
            - Disposes PowerShell + AsyncWaitHandle immediately on completion
            - Prevents unbounded RAM growth on large endpoint sets

        HARDENING: HEC timestamp + dead-letter
            - ParseExact("o") timestamp parsing with fallback
            - Optional hec.dead_letter_path to persist failed batch payloads when dropping on failure
            - Optional simplified knobs: hec.retry_count and hec.retry_delay_ms

        DIAGNOSTICS: debug.emit_memory_stats
            - Alias to diagnostics output for quick memory proofing
...
.PARAMETER MaxCycles
    Maximum number of cycles to run before exiting. Defaults to 0 (unlimited).
...
function Get-Configuration {
    ...
    # Defaults (v4.0.0 maintains backward compatibility with v3.x config keys)
    $defaults = @{
        ...
        # v4.0.0: Simple debug toggle (alias to diagnostics) for environments that only want memory stats
        ...
        hec = @{
            ...
            # v4.0.0: Optional simplified knobs (do not replace retry block; they only provide an alternate config surface)
            retry_count = 0
            retry_delay_ms = 250
            dead_letter_path = ""
        }
    ...
    # Merge debug defaults (v4.0.0)
    if ($config.ContainsKey('debug') -and $null -ne $config.debug) {
        foreach ($bk in $defaults.debug.Keys) {
            if (-not $config.debug.ContainsKey($bk)) { $config.debug[$bk] = $defaults.debug[$bk] }
        }
    } else { $config['debug'] = $defaults.debug }
...
function Write-JsonLinesToWriter {
    ...
    $isEnumerable = ($Results -is [System.Collections.IEnumerable]) -and
        -not ($Results -is [string]) -and
        -not ($Results -is [hashtable]) -and
        -not ($Results -is [System.Collections.IDictionary])

    if ($isEnumerable) {
        foreach ($r in $Results) { Write-JsonLineToWriter -Writer $Writer -Event $r }
    } else {
        Write-JsonLineToWriter -Writer $Writer -Event $Results
    }
}
...
function Initialize-HecBuffer {
    ...
    # v4.0.0: Support simplified retry knobs (retry_count / retry_delay_ms) in addition to the existing retry block.
    $simpleRetryCount = 0
    $simpleRetryDelayMs = 250
    $deadLetterPath = ""
    try {
        if ($HecConfig.ContainsKey('retry_count')) { $simpleRetryCount = [int]$HecConfig.retry_count }
        if ($HecConfig.ContainsKey('retry_delay_ms')) { $simpleRetryDelayMs = [int]$HecConfig.retry_delay_ms }
        if ($HecConfig.ContainsKey('dead_letter_path')) { $deadLetterPath = [string]$HecConfig.dead_letter_path }
    } catch { }

    $blockRetryEnabled = $false
    try { $blockRetryEnabled = [bool]$HecConfig.retry.enabled } catch { $blockRetryEnabled = $false }

    $effectiveRetryEnabled = $blockRetryEnabled -or ($simpleRetryCount -gt 0)
    $effectiveMaxAttempts = if ($blockRetryEnabled) { [int]$HecConfig.retry.max_attempts } elseif ($simpleRetryCount -gt 0) { [math]::Max(1, $simpleRetryCount + 1) } else { 1 }
    $effectiveBaseDelayMs = if ($blockRetryEnabled) { [int]$HecConfig.retry.base_delay_ms } elseif ($simpleRetryCount -gt 0) { [math]::Max(0, $simpleRetryDelayMs) } else { 0 }
    $effectiveJitterPct = if ($blockRetryEnabled) { [int]$HecConfig.retry.jitter_pct } else { 0 }
    $effectiveBackoff = if ($blockRetryEnabled) { $HecConfig.retry.backoff } else { 'fixed' }
    ...
    return @{
        ...
        RetryEnabled = [bool]$effectiveRetryEnabled
        RetryMaxAttempts = [int]$effectiveMaxAttempts
        RetryBaseDelayMs = [int]$effectiveBaseDelayMs
        RetryJitterPct = [int]$effectiveJitterPct
        RetryBackoff = $effectiveBackoff
...
function Convert-IsoTimestampToUnixSeconds {
    <# v4.0.0: Robust ISO-8601 "o" timestamp parsing with fallback #>
    param([string]$Timestamp)
    if ([string]::IsNullOrWhiteSpace($Timestamp)) { return [DateTimeOffset]::UtcNow.ToUnixTimeSeconds() }
    try {
        return [DateTimeOffset]::ParseExact($Timestamp, 'o', [System.Globalization.CultureInfo]::InvariantCulture).ToUnixTimeSeconds()
    } catch {
        try { return [DateTimeOffset]::Parse($Timestamp).ToUnixTimeSeconds() } catch { return [DateTimeOffset]::UtcNow.ToUnixTimeSeconds() }
    }
}
...
function Add-EventToHecBuffer {
    ...
    # v4.0.0: Prefer ParseExact("o") then fallback; do not allow malformed timestamps to break emission.
    $unixTime = Convert-IsoTimestampToUnixSeconds -Timestamp $Event.timestamp
...
function Add-EventsToHecBuffer {
    ...
    if ($null -eq $Events) { return @{ SuccessCount = 0; FailCount = 0 } }

    $isEnumerable = ($Events -is [System.Collections.IEnumerable]) -and
        -not ($Events -is [string]) -and
        -not ($Events -is [hashtable]) -and
        -not ($Events -is [System.Collections.IDictionary])

    $eventList = if ($isEnumerable) { $Events } else { @($Events) }
...
function Flush-HecBuffer {
    ...
    if ($success -or $Buffer['DropOnFailure']) {
        # v4.0.0: If we're dropping on failure and dead-letter is configured, write the payload for later replay.
        if (-not $success -and $Buffer['DropOnFailure']) {
            $dlp = $Buffer['DeadLetterPath']
            if (-not [string]::IsNullOrWhiteSpace($dlp)) {
                try {
                    $dlDir = Split-Path -Parent $dlp
                    if (-not [string]::IsNullOrWhiteSpace($dlDir)) { [System.IO.Directory]::CreateDirectory($dlDir) | Out-Null }
                    # Append the raw NDJSON payload (same format expected by Splunk HEC)
                    Add-Content -Path $dlp -Value ($body + "`n") -Encoding UTF8
                    Write-Warning "HEC dead-letter: wrote failed batch payload to '$dlp'"
                } catch {
                    Write-Warning "HEC dead-letter write failed: $($_.Exception.Message)"
                }
            }
        }
...
function Build-MetricsPayload {
    ...
    $unixTime = Convert-IsoTimestampToUnixSeconds -Timestamp $Summary.timestamp
...
function Invoke-StreamingParallelPing {
    ...
    $pingScript = {
        ...
        # Use a PowerShell array for compatibility across runspaces.
        # NOTE: Avoid `if (...) { @() }` as an expression because an empty array emits no pipeline output and assigns `$null`.
        $individual = $null
        if ($EmitIndividual) { $individual = @() }
        ...
        # Marshal individual pings as an array for consistency (including the single-element case).
        $individualOut = if ($null -ne $individual) { @($individual) } else { $null }
        return @{ Individual=$individualOut; Summary=$summary }
    }

    # v4.0.0: Bounded scheduler — do NOT start all endpoints at once.
    # Keep only up to RunspacePool.MaxRunspaces async operations in-flight.
    $maxInFlight = 1
    try { $maxInFlight = [int]$RunspacePool.GetMaxRunspaces() } catch {
        try { $maxInFlight = [int]$RunspacePool.MaxRunspaces } catch { $maxInFlight = 1 }
    }
    if ($maxInFlight -lt 1) { $maxInFlight = 1 }
    if ($maxInFlight -gt $Endpoints.Count) { $maxInFlight = $Endpoints.Count }

    $queue = [System.Collections.Generic.Queue[PSCustomObject]]::new()
    foreach ($ep in $Endpoints) { $queue.Enqueue($ep) }

    $pingScriptText = $pingScript.ToString()

    function Start-NextEndpoint {
        param()
        if ($queue.Count -le 0) { return $false }
        $ep = $queue.Dequeue()
        $ps = $null
        try {
            $ps = [powershell]::Create()
            $ps.RunspacePool = $RunspacePool
            [void]$ps.AddScript($pingScriptText).AddArgument($ep).AddArgument($PingsPerCycle).AddArgument($TimeoutMs).AddArgument($EmitIndividualPings).AddArgument($hostname)
            $handle = $ps.BeginInvoke()
            $activeRunspaces.Add(@{ PowerShell=$ps; Handle=$handle; Endpoint=$ep })
            return $true
        } catch {
            try { if ($null -ne $ps) { $ps.Dispose() } } catch { }
            Write-Warning "Failed to start runspace for $($ep.hostname): $($_.Exception.Message)"
            $stats.TotalFailed++
            return $false
        }
    }

    while ($activeRunspaces.Count -lt $maxInFlight -and $queue.Count -gt 0) { [void](Start-NextEndpoint) }

    # Process as they complete, immediately starting the next endpoint (true streaming, bounded memory)
    while ($activeRunspaces.Count -gt 0) {
        $completed = [System.Collections.Generic.List[int]]::new()

        if ($activeRunspaces.Count -le 64) {
            # Prefer WaitAny to avoid O(n) busy polling; safe because we cap in-flight size.
            $handles = [System.Threading.WaitHandle[]]::new($activeRunspaces.Count)
            for ($i = 0; $i -lt $activeRunspaces.Count; $i++) {
                $handles[$i] = $activeRunspaces[$i].Handle.AsyncWaitHandle
            }
            $signaled = [System.Threading.WaitHandle]::WaitAny($handles, 250)
            if ($signaled -ne [System.Threading.WaitHandle]::WaitTimeout) {
                $completed.Add([int]$signaled)
            }
            # Opportunistically collect any other completed items.
            for ($i = 0; $i -lt $activeRunspaces.Count; $i++) {
                if ($activeRunspaces[$i].Handle.IsCompleted -and -not $completed.Contains($i)) { $completed.Add($i) }
            }
        } else {
            # Fallback for very large parallelism (WaitAny supports max 64 handles)
            for ($i = 0; $i -lt $activeRunspaces.Count; $i++) {
                if ($activeRunspaces[$i].Handle.IsCompleted) { $completed.Add($i) }
            }
            if ($completed.Count -eq 0) { Start-Sleep -Milliseconds 10; continue }
        }

        if ($completed.Count -eq 0) { continue }

        $completedIdx = @($completed.ToArray() | Sort-Object -Descending)
        foreach ($idx in $completedIdx) {
            ...
            try {
                ...
                Write-Warning "Runspace error for $($rs.Endpoint.hostname): $($_.Exception.Message)"
                $stats.TotalFailed++
                $endInvokeCalled = $true
            }
            finally {
...
function Start-PingMonitor {
    ...
    Write-Host "  Splunk Ping Monitor v4.0.0" -ForegroundColor Cyan
    Write-Host "  (Bounded parallel scheduler + memory stability)" -ForegroundColor DarkCyan
    Write-Host "========================================" -ForegroundColor Cyan
    ...
    try { $diagnosticsEnabled = [bool]$Config.diagnostics.enabled } catch { $diagnosticsEnabled = $false }
    try {
        if ($Config.ContainsKey('debug') -and $null -ne $Config.debug -and [bool]$Config.debug.emit_memory_stats) {
    ...
    try {
        do {
            ...
            $shouldContinue = (-not $RunOnce) -and (($MaxCycles -le 0) -or ($cycleCount -lt $MaxCycles))
            if ($shouldContinue) {
        ...
        } while ((-not $RunOnce) -and (($MaxCycles -le 0) -or ($cycleCount -lt $MaxCycles)))
    }
    finally {
    ...
    Write-Host "Ping Monitor v4.0.0 completed." -ForegroundColor Cyan
}
...
try {
    ...
    Start-PingMonitor -Config $config -Endpoints $endpoints -RunOnce:$RunOnce -MaxCycles $MaxCycles
}
catch {
